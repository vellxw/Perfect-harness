...
const output=resolve(process.argv[2]??'test-results/tui');await mkdir(output,{recursive:true});
const escape=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
const color=c=>typeof c?.toHex==='function'?c.toHex():typeof c?.toString==='function'&&/^#/.test(c.toString())?c.toString():'#adb6ca';
const results=[];
for(const [name,scene,screen,cols,rows] of [
 ['idle','idle','home',120,36],['running','running','home',160,45],['concurrent','concurrent','home',160,45],['repair','repair','home',120,36],['compact','repair','home',80,24],['agents','running','agents',120,36],['plan','running','plan',120,36],['verification','repair','verify',120,36],['routing','running','routing',120,36],['paused','paused','home',120,36],['done','done','home',120,36],['settings','idle','settings',120,36]]){
 const test=await createTestRenderer({width:cols,height:rows}),root=createRoot(test.renderer),client=new DemoClient(scene);
 root.render(createElement(App,{client,initialScreen:screen,onExit:()=>{}}));
 await new Promise(r=>setTimeout(r,100));await test.renderOnce();
 const frame=test.captureCharFrame(),spans=test.captureSpans();
 await writeFile(join(output,name+'.txt'),frame);
 const cellW=10,cellH=20,pad=24,label=30,width=cols*cellW+pad*2,height=rows*cellH+pad*2+label;
...
await writeFile(join(output,'provenance.json'),JSON.stringify({node:process.version,platform:process.platform,opentui:'0.5.9',captures:results},null,2));
