...
patch('src/cli/login.ts','`Open: ${event.url}\\n${event.instructions ?? ""}`','`Abrí: ${event.url}\\n${humanMessage(event.instructions ?? "")}`');
patch('src/cli/login.ts','console.log(event.message)','console.log(humanMessage(event.message))');
patch('src/cli/login.ts','${option.label}${option.description ? ` — ${option.description}` : ""}','${humanMessage(option.label)}${option.description ? ` — ${humanMessage(option.description)}` : ""}');
patch('src/presentation/engine.ts','{ id: o.id, label: o.label }','{ id: o.id, label: humanMessage(o.label) }');
patch('src/presentation/engine.ts','${e.kind} · ${e.contentHash}','${valueLabel(e.kind)} · ${e.contentHash}');
prepend('src/presentation/engine.ts','import { valueLabel } from "../i18n/es.js";');
patch('src/ui/tui/app.tsx','name = canonicalCommand(name);','name = canonicalCommand(name);\n    if (name === "status") name = "home";');
patch('src/ui/tui/app.tsx','/agents    /plan    /verify    /diff','/agentes   /plan   /verificar   /cambios',false);
patch('src/ui/tui/app.tsx','${roles[item.role]?.name ?? item.role}','${roles[item.role]?.name ?? roleLabel(item.role)}',false);
...
for(const [a,b] of [
 ['lang="en"','lang="es"'],['Reserve / Perfect Harness Demo','Reservas / Demostración de Perfect Harness'],['RESERVE<span','RESERVAS<span'],['A PERFECT HARNESS DEMO','DEMOSTRACIÓN DE PERFECT HARNESS'],['YOUR TIME, WELL SPENT','TU TIEMPO, BIEN APROVECHADO'],['A little space<br>in your day.','Un espacio<br>en tu día.'],['Choose an available time. We will take care of saving your spot.','Elegí un horario disponible. Nos encargamos de guardar tu lugar.'],['Live availability · Secure reservation','Disponibilidad actual · Reserva segura'],['BOOK YOUR VISIT','RESERVÁ TU VISITA'],['Find your moment','Encontrá tu momento'],['Available time','Horario disponible'],['Loading available times…','Cargando horarios disponibles…'],['Your name','Tu nombre'],['Reserve this time','Reservar este horario'],['One reservation per time slot. No double bookings.','Una reserva por horario. Sin reservas duplicadas.'],['Built and verified in an isolated workspace. Demo only; no production authentication.','Construido y verificado en una carpeta aislada. Solo demostración; sin autenticación de producción.'],['Availability could not be loaded','No se pudo cargar la disponibilidad'],['Saving your reservation…','Guardando tu reserva…'],['Reservation failed','La reserva falló'],['Reservation confirmed for ','Reserva confirmada para '],
])patch(frontend,a,b);
