...
change('src/desktop/main/index.ts','const [w,h]=window.getContentSize();','const [w=0,h=0]=window.getContentSize();');
change('src/desktop/main/index.ts','width:window.getContentSize()[0]-32,height:window.getContentSize()[1]-190','width:(window.getContentSize()[0]??900)-32,height:(window.getContentSize()[1]??600)-190');
...
change('src/desktop/main/broker.ts','requestId=randomUUID()','requestId:string=randomUUID()');
change('src/desktop/engine/worker.ts','import { GitWorkspace }','import { GitWorkspace, manifest }');
change('src/desktop/engine/worker.ts','import { listSafeFiles, safePath }','import { safePath }');
{const p='src/desktop/engine/worker.ts',s=fs.readFileSync(p,'utf8');fs.writeFileSync(p,s.replaceAll('await listSafeFiles(root,config)','(await manifest(root,config)).files.map(f=>f.path)'));}
