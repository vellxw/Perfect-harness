"""Optional developer-side pixel review. No model enters the product payload.

This produces observations, NOT an owner approval or a release decision.
All inference runs locally on synthetic, hash-checked acceptance evidence.
"""
...
def main() -> None:
    ...
    parser.add_argument("--max-images", type=int, default=28)
    ...
    images = select_images(acceptance["files"])
    if not 1 <= args.max_images <= 28:
        raise ValueError("Explicit image budget is required")
    ...
    # Record exact local weight files; no arbitrary repository Python is executed.
    ...
    model.eval()
    # Inference sees local pixels only. Deny outbound sockets after preparation.
    ...
    report = {
        ...
        "limitations": "Auxiliary model observations, not owner approval. Screenshots and five samples per video do not prove continuous motion or correctness between samples. Real browser/native interaction, full video decoding and measured motion are separate evidence. The orchestrating chat did not directly see these pixels.",
    }
