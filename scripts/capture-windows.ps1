...
$exe=Join-Path $packageRoot 'Perfect.exe'
$temp=Join-Path $env:RUNNER_TEMP ('perfect-terminal-'+[Guid]::NewGuid())
New-Item -ItemType Directory $temp -Force | Out-Null
$os=Get-CimInstance Win32_OperatingSystem
$report=@{os=$os.Caption;build=$os.BuildNumber;terminal='1.24.11911.0';mode='real Windows Terminal window; synthetic app fixture, no OAuth';status='NOT_TESTED';captures=@();error=$null;sourceCommit=$env:GITHUB_SHA;installedPackage=$true}
...
try {
 ...
 if($LASTEXITCODE -ne 0){throw 'Installed executable could not print its profile'}
 ...
 foreach($scene in @('idle','running')){
   ...
   [void][PerfectCaptureWin32]::MoveWindow($window.MainWindowHandle,$screen.Left+12,$screen.Top+12,[Math]::Min(1380,$screen.Width-24),[Math]::Min(880,$screen.Height-24),$true)
   [void][PerfectCaptureWin32]::SetForegroundWindow($window.MainWindowHandle)
   Start-Sleep -Seconds 5
   $children=@(Get-CimInstance Win32_Process | Where-Object {$_.Name -eq 'node.exe' -and $_.CommandLine -and ($_.CommandLine.IndexOf($packageRoot,[StringComparison]::OrdinalIgnoreCase) -ge 0) -and $_.CommandLine -like "*--demo*$scene*"})
   if(-not $children.Count){throw 'The installed TUI process is not alive; screenshot is not accepted'}
   ...
   try {
     ...
     $report.captures+=@{file=$name;scene=$scene;width=$w;height=$h;processVerified=$true;inspection='Actual pixel capture; independent inspection is needed for visual acceptance'}
