...
async function replaceOnce(path, before, after) {
  ...
  if (!text.includes(before))
    throw Error("Expected source fragment missing in " + path);
  if (text.indexOf(before) !== text.lastIndexOf(before))
    throw Error("Ambiguous source replacement in " + path);
  await writeFile(path, text.replace(before, after));
}
await replaceOnce(
  "src/examples/reservation-tests.ts",
...
await writeFile(
  "perfect.config.example.json",
  JSON.stringify(defaultConfig(), null, 2) + "\n",
);
await writeFile(
  "perfect.config.schema.json",
