...
export const GithubSkillSchema = z
  .object({
...
export async function publicBytes(
  ...
  if (
    ...
    throw new Blocked(
      "SKILL_REMOTE_ORIGIN",
      "Solo se admite GitHub público sin credenciales",
    );
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "Perfect-Harness-skills",
    },
  });
  if (!response.ok) {
    ...
    throw new Blocked(
      "SKILL_FETCH",
      `GitHub respondió ${response.status}; no se usó una cuenta personal`,
    );
  }
  if (Number(response.headers.get("content-length")) > limit) {
    await response.body?.cancel();
    throw new Blocked("SKILL_REMOTE_SIZE", "Respuesta demasiado grande");
  }
  const chunks: Uint8Array[] = [];
  let total = 0;
  if (!response.body)
    throw new Blocked("SKILL_REMOTE_EMPTY", "Respuesta vacía");
...
function recognizedLicense(text: string): string {
  ...
  if (/Apache License/i.test(text) && /Version 2\.0/i.test(text))
    return "Apache-2.0";
  if (
    /Creative Commons Attribution-ShareAlike 4\.0|CC-BY-SA-4\.0|Attribution-ShareAlike 4\.0 International/i.test(
      text,
    )
  )
    return "CC-BY-SA-4.0";
  if (
    /ISC License/i.test(text) &&
    /Permission to use, copy, modify/i.test(text)
  )
    return "ISC";
  return "unknown";
}
...
export async function importGithubSkill(
  raw: unknown,
  signal: AbortSignal,
  fetchBytes: typeof publicBytes = publicBytes,
): Promise<SkillRelease> {
  const input = GithubSkillSchema.parse(raw);
  safeResource(input.path);
  if (/supabase/i.test(input.repository))
    throw new Blocked("SKILL_EXCLUDED", "Supabase está excluido");
  if (input.repository.toLowerCase() === "chuspeeism/dashi-motion")
    throw new Blocked(
      "SKILL_LICENSE",
      "Dashi está pendiente de licencia; no se copia automáticamente",
    );
  const transitions =
    input.repository.toLowerCase() === "jakubantalik/transitions.dev";
  if (
    transitions &&
    (!input.localOnly || input.path !== "skills/transitions-dev")
  )
    throw new Blocked(
      "SKILL_TRANSITIONS_SCOPE",
      "Solo se admite la skill gratuita anunciada, para uso local",
    );
  ...
  const tree = TreeSchema.parse(
    JSON.parse(
      (
        await fetchBytes(
          `https://api.github.com/repos/${input.repository}/git/trees/${input.commit}?recursive=1`,
          signal,
          8_000_000,
        )
      ).toString("utf8"),
  ...
  if (tree.truncated)
    throw new Blocked(
      ...
      "No se puede comprobar un árbol incompleto",
    );
  ...
  if (
    ...
    entries.length > 120 ||
    entries.some(
      (e) => e.type !== "blob" || !["100644", "100755"].includes(e.mode),
    )
  )
    throw new Blocked(
      "SKILL_REMOTE_LAYOUT",
      "Paquete vacío, excesivo o con enlaces/submódulos",
    );
  ...
  >;
  let total = 0;
  const rawFile = (path: string) =>
    `https://raw.githubusercontent.com/${input.repository}/${input.commit}/${path.split("/").map(encodeURIComponent).join("/")}`;
  for (const entry of entries) {
    const name = safeResource(entry.path.slice(prefix.length));
    if (transitions && /(?:^|\/)pro(?:[-_/]|$)/i.test(name))
      throw new Blocked(
        "SKILL_PREMIUM_DENIED",
  ...
  for (const [path, bytes] of Object.entries(files))
    if (/^(?:LICENSE|COPYING)(?:\.[a-z]+)?$/i.test(path))
      licenseTexts.push(bytes.toString("utf8"));
  let license = recognizedLicense(licenseTexts.join("\n"));
  let adaptation: string | undefined;
  if (transitions) {
    license =
      "Transitions.dev free recipes — local use, no collection redistribution";
    files["TERMS-SOURCE.txt"] = Buffer.from(
    ...
    try {
      parseSkill(original.toString("utf8"));
    } catch {
      ...
      adaptation =
        "La cabecera upstream no satisface el límite del estándar; se conserva el original íntegro como recurso local y se utiliza una entrada corta identificada.";
    }
  }
  return releaseFromFiles({
    ...
    provenance: {
      kind: "github",
      source: `https://github.com/${input.repository}`,
      commit: input.commit,
      path: input.path,
      license,
      redistribution:
    ...
    packageId: transitions
      ? "transitions-dev-free"
      : input.repository.toLowerCase() === "obra/superpowers"
        ? "superpowers"
        : undefined,
  });
}
