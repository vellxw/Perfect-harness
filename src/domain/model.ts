...
export const CriterionSchema = z
  .object({
    ...
    kind: z
      .enum(["functional", "visual", "security", "human"])
      .default("functional"),
  })
  .strict();
...
export const CommandSchema = z
  .object({
    ...
    timeoutMs: z.number().int().positive().max(1_200_000).default(120_000),
  })
  .strict();
...
export const VisualScenarioSchema = z
  .object({
...
export const VerificationSpecSchema = z
  ...
  .superRefine((s, ctx) => {
    ...
    if (s.kind === "remotion" && !s.remotion)
      ctx.addIssue({
        code: "custom",
        message: "Remotion specification required",
      });
  });
...
export const TaskSpecSchema = z
  .object({
    ...
    verificationStrategy: z.array(Id).default([]),
...
export const PlanProposalSchema = z
  .object({
    ...
    tasks: z.array(TaskSpecSchema).min(1).max(100),
  })
  .strict();
export type PlanProposal = z.infer<typeof PlanProposalSchema>;
export interface Plan extends PlanProposal {
  id: string;
  goalId: string;
  version: number;
  parentPlanId?: string;
...
export interface AgentDefinition {
  ...
  model: string;
  reasoning: Reasoning;
  auth: "oauth" | "api_key" | "mock";
  billingMode: "subscription" | "free" | "metered" | "mock";
  capabilities: ("text" | "image" | "tools")[];
  readOnly: boolean;
}
export interface RouteBinding extends AgentDefinition {
...
export interface Usage {
  ...
  retryCount: number;
  billingMode: AgentDefinition["billingMode"];
  estimatedCost?: number;
  reportedCharge?: number;
  completeness: "reported" | "partial" | "unknown";
  createdAt: string;
}
...
export interface ContextPackage {
  ...
  manifest: string[];
  dependencyOutputs: { taskId: string; revision: string; summary: string }[];
  contracts: string[];
  failures: Failure[];
  evidence: Evidence[];
...
export interface Evidence {
  ...
  artifactRef: string;
  contentHash: string;
  producer: "runner" | "reviewer" | "controller" | "human";
  revision: string;
  environmentHash: string;
  criteriaIds: string[];
  capturedAt: string;
  validity: "valid" | "stale" | "rejected";
}
...
export const ReviewSchema = z
  .object({
    decision: z.enum(["approve", "request_changes", "uncertain"]),
...
export interface Task extends TaskSpec {
  ...
  model: string;
  reasoning: Reasoning;
  resolvedRouteBinding?: RouteBinding;
  baseRevision: string;
  dependencyOutputVersions: Record<string, string>;
  worktreeRef?: string;
  attempt: number;
  maxAttempts: number;
  outputs: string[];
...
export interface Approval {
  id: string;
  goalId: string;
  kind: "contributor" | "plan" | "human" | "apply" | "network";
  scopeHash: string;
  approvedAt: string;
  actor: "user";
}
export interface OperationIntent {
  ...
  goalId: string;
  taskId?: string;
  kind: "container" | "integration" | "apply";
