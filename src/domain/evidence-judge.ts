...
export function judge(input: {
  ...
  if (
    !tasks.length ||
    tasks.some((t) => !["accepted", "superseded"].includes(t.status))
  )
    reasons.push("Necessary tasks remain unfinished");
  ...
  for (const spec of plan.verification.filter((s) => s.mandatory)) {
    ...
    if (
      ...
      result.evidenceIds.some(
        (eid) => !valid.some((e) => e.id === eid && e.producer === "runner"),
      )
    )
  ...
  for (const criterion of plan.criteria.filter((c) => c.mandatory)) {
    if (criterion.kind === "human") {
      ...
      !valid.some(
        (e) => e.criteriaIds.includes(criterion.id) && e.producer === "runner",
      )
    )
      reasons.push(`No runner evidence for: ${criterion.id}`);
  }
  for (const role of input.requiredReviews) {
    ...
    if (
      ...
      review.evidenceIds.some(
        (eid) => !valid.some((e) => e.id === eid && e.producer === "reviewer"),
      )
    )
