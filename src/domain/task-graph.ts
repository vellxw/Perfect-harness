...
export function validatePlan(input:unknown,previous?:PlanProposal):PlanProposal {
  ...
  const normalized=input&&typeof input==='object'&&!Array.isArray(input)?Object.fromEntries(Object.entries(input).filter(([,value])=>value!==undefined)):input;
  const plan=PlanProposalSchema.parse(normalized);
  const criteria=new Set(plan.criteria.map(c=>c.id)),checks=new Set(plan.verification.map(v=>v.id)),ids=new Set(plan.tasks.map(t=>t.id));
  ...
  const visit=(taskId:string)=>{if(active.has(taskId))throw new Error('Task graph contains a cycle');if(visited.has(taskId))return;active.add(taskId);for(const dep of plan.tasks.find(t=>t.id===taskId)!.dependencies)visit(dep);active.delete(taskId);visited.add(taskId);};
