...
export function emptySnapshot(workspace = ""): UiSnapshot {
  return {
    ...
    recentGoals: [],
    diagnostics: [],
    preferences: UiPreferencesSchema.parse({}),
  };
}
...
export class EngineClient implements UiClient {
  ...
  constructor(options: { workspace?: string; home?: string }) {
    ...
    this.child = fork(worker, [], {
      ...
      execArgv: process.execArgv.filter(
        (a) => !a.startsWith("--inspect") && !a.includes("experimental-ffi"),
      ),
    });
    this.child.stdout?.resume();
    this.child.stderr?.on("data", () => {});
    this.child.on("message", (raw) => {
      const m = raw as UiMessage;
      if (m.type === "snapshot" && m.snapshot?.protocol === 1) {
        this.state = m.snapshot;
        for (const l of this.listeners) l();
      } else for (const l of this.messages) l(m);
    });
    this.child.on("error", (error) => this.fail(text(error)));
    this.child.on("exit", (code) => {
      if (!this.exiting)
        this.fail(
          `Engine disconnected (exit ${code ?? "unknown"}). No success was inferred; reopen Perfect to recover.`,
        );
    });
  ...
  private fail(message: string) {
    ...
    for (const l of this.listeners) l();
    for (const l of this.messages) l({ type: "fault", message });
  }
  ...
  onMessage = (listener: (message: UiMessage) => void) => {
