...
export class EngineClient implements UiClient {
 ...
 constructor(options:{workspace?:string;home?:string}){
  ...
  const execArgv=process.execArgv.filter(a=>!a.startsWith("--inspect")&&!a.includes("experimental-ffi"));
  this.child=spawn(process.execPath,[...execArgv,worker],{stdio:["ignore","pipe","pipe","ipc"],windowsHide:true,shell:false});
  ...
  this.child.on("exit",code=>{if(!this.exiting)this.fail(`Engine disconnected (exit ${code??"unknown"}). Reopen Perfect to recover; no success was inferred.`);});
