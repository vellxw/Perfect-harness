    ...
    session.subscribe(event=>{
      try{
        ...
        if(event.type==='message_end'&&event.message.role==='assistant'){
          ...
          if(metadata.responseModel&&metadata.responseModel!==route.model)auditError=`ROUTE_RESPONSE_MISMATCH: ${metadata.responseModel}`;
          if(metadata.providerThinkingLevel&&metadata.providerThinkingLevel!==nativeReasoning)auditError=`REASONING_RESPONSE_MISMATCH: ${metadata.providerThinkingLevel}`;
          ...
          if(current){
            ...
            const usage:Usage={id:id('usage'),goalId:run.goalId,runId:run.id,requestId:current.id,provider:route.provider,accountRef:route.accountRef,modelRequested:route.model,modelSerialized:current.serialized,modelReported:metadata.responseModel,reasoningRequested:route.reasoning,reasoningSent:current.reasoning,reasoningReported:metadata.providerThinkingLevel,
              inputTokens:known?u.input:undefined,outputTokens:known?u.output:undefined,cacheReadTokens:known?u.cacheRead:undefined,cacheWriteTokens:known?u.cacheWrite:undefined,reasoningTokens:known?u.reasoning:undefined,totalTokens:known?u.totalTokens:undefined,latencyMs:Date.now()-current.started,timeToFirstTokenMs:current.firstToken===undefined?undefined:current.firstToken-current.started,retryCount:retries,billingMode:route.billingMode,estimatedCost:known&&route.billingMode==='metered'?u.cost.total:undefined,completeness:known?'reported':'unknown',createdAt:now()};
