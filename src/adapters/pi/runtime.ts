...
): { model: string; reasoning?: string; body: Record<string, unknown> } {
  if (!payload || typeof payload !== "object")
    throw new Blocked("ROUTE_PAYLOAD", "Provider payload is not observable");
  const body = payload as Record<string, unknown>;
  if (body.model !== route.model)
    throw new Blocked(
      "ROUTE_MODEL_MISMATCH",
      `Expected ${route.model}, serialized ${String(body.model)}`,
    );
  const reasoningObject = body.reasoning as { effort?: unknown } | undefined;
  const reasoning =
    typeof body.reasoning_effort === "string"
      ? body.reasoning_effort
      : typeof reasoningObject?.effort === "string"
        ? reasoningObject.effort
        : undefined;
  if (route.reasoning !== "off" && reasoning !== nativeReasoning)
    throw new Blocked(
      "REASONING_MISMATCH",
      `Expected ${nativeReasoning}, serialized ${reasoning ?? "not observable"}`,
    );
...
export class PiRuntime implements AgentRuntime {
  constructor(
    ...
    private factory?: (def: AgentDefinition) => Promise<ModelRuntime>,
  ) {}
  ...
  async resolve(
    ...
    if (
      def.model.includes("contributor") &&
      (privacy !== "public" || !contributorConsent)
    )
      throw new Blocked(
        "CONTRIBUTOR_CONSENT",
        "Contributor routes require public classification and explicit workspace consent",
      );
    if (def.billingMode === "metered" && !this.config.budgets.allowMetered)
      throw new Blocked("METERED_DENIED", def.id);
    const runtime = await this.modelRuntime(def),
      model = runtime.getModel(def.provider, def.model);
    if (!model)
    ...
    if (
      def.reasoning !== "off" &&
      (!model.reasoning ||
        map?.[def.reasoning] === null ||
        (["xhigh", "max"].includes(def.reasoning) && !map?.[def.reasoning]))
    )
      throw new Blocked(
        "REASONING_UNSUPPORTED",
        `${def.model}/${def.reasoning}`,
      );
    ...
    if (!status)
      throw new Blocked("AUTH_REQUIRED", `Run perfect login ${def.provider}`);
    if (def.auth !== "mock" && status.type !== def.auth)
      throw new Blocked(
        "AUTH_ROUTE_MISMATCH",
        `Required ${def.auth}; configured ${status.type}`,
      );
    if (def.billingMode === "subscription" && def.auth !== "oauth")
      throw new Blocked(
        "BILLING_MISMATCH",
        "Subscription route must use OAuth",
      );
    if (
    ...
    return {
      ...
      selectedReasoning: def.reasoning,
      resolvedAt: now(),
      provenance: this.factory ? "mock" : "catalog",
    };
  }
  ...
  private async runWithIntegrations(
    ...
    const runtime = await this.modelRuntime(route),
      model = runtime.getModel(route.provider, route.model);
    if (!model) throw new Blocked("MODEL_UNAVAILABLE", route.model);
    const nativeReasoning =
      route.reasoning === "off"
        ? undefined
        : (model.thinkingLevelMap?.[route.reasoning] ?? route.reasoning);
    let submitted = false,
      result: unknown,
      lastError = "",
      auditError = "",
      ...
      return original(m, context, {
        ...
        onPayload: async (payload, selectedModel) => {
          ...
          if (++requestCount > this.config.limits.maxAgentTurns)
            throw new Blocked("TURN_LIMIT", "Agent turn limit reached");
          const transformed =
            (await options?.onPayload?.(payload, selectedModel)) ?? payload;
          const observed = auditPayload(transformed, route, nativeReasoning);
          const cap =
            observed.body.max_output_tokens ??
            observed.body.max_completion_tokens ??
            observed.body.max_tokens;
          if (
          ...
          request.event("provider.payload", {
            requestId,
            model: observed.model,
            reasoning: observed.reasoning,
            endpoint: route.endpoint,
            payloadHash: hash(transformed),
          });
          return transformed;
        },
    ...
    session.subscribe((event) => {
      try {
        if (
          ...
          current &&
          current.firstToken === undefined
        )
          current.firstToken = Date.now();
        if (event.type === "tool_execution_end")
          request.event("agent.tool_completed", {
          ...
          if (current) {
            ...
            const usage: Usage = {
              ...
              timeToFirstTokenMs:
                current.firstToken === undefined
                  ? undefined
                  : current.firstToken - current.started,
              retryCount: retries,
              billingMode: route.billingMode,
              estimatedCost:
                known && route.billingMode === "metered"
                  ? u.cost.total
                  : undefined,
              completeness: known ? "reported" : "unknown",
              createdAt: now(),
            };
            request.usage(usage);
            current = undefined;
        ...
        auditError = `AUDIT_FAILURE: ${errorText(error)}`;
        void session.abort();
      }
    });
    try {
      ...
      while (true) {
        ...
        const transient =
          /429|rate.?limit|timed? ?out|ECONNRESET|temporar|\b50[234]\b/i.test(
            lastError,
          ) &&
          !/quota|exhausted|ROUTE_|REASONING_|BUDGET_|TURN_LIMIT/i.test(
            lastError,
          );
        ...
        await delay(
          retryAfter || Math.min(10000, 500 * 2 ** retries),
          undefined,
          { signal },
        );
        prompt =
          "Continue after a transient provider interruption. Inspect current files before repeating tools.";
      }
      if (!submitted)
        throw new Error(
