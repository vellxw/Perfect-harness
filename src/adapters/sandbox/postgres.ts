...
export class PostgresRunner {
  ...
  async run(
    ...
    if (!(await runtime.available()))
      throw new Blocked(
        "SANDBOX_REQUIRED",
        "PostgreSQL se verifica en contenedores, nunca contra una base del host",
      );
    const appImage = await preparedImage(
      request.workspace,
      request.goalId,
    ...
    for (const image of [POSTGRES_IMAGE, appImage]) {
      ...
      if (
        ...
        throw new Blocked("SANDBOX_IMAGE", `Prepará explícitamente ${image}`);
      imageIds[image] = inspected.stdout.trim();
    }
    await mkdir(this.directory, { recursive: true, mode: 0o700 });
    const dir = await mkdtemp(join(this.directory, "postgres-"));
    if (/[:,\n]/.test(dir))
      throw new Blocked("MOUNT_PATH", "Ruta incompatible con montaje Docker");
    ...
    const network = id("perfect-pg-net"),
      database = id("perfect-pg-db"),
      app = id("perfect-pg-check"),
    ...
    const intent = (resource: string) => {
      ...
      this.store.put("intents", value, "postgres.resource_started");
      intents.push(value);
    };
    const run = (args: string[], timeoutMs = 30000) =>
      processRun("docker", args, { signal: request.signal, timeoutMs });
    try {
      ...
      if (net.code)
        throw new Blocked(
          "POSTGRES_NETWORK",
          "No se pudo crear red interna de prueba",
        );
      intent(database);
      const db = await run([
        "run",
        ...
        "--cpus=1",
        "--tmpfs",
        "/var/lib/postgresql/data:rw,nosuid,nodev,uid=999,gid=999,mode=0700,size=256m",
        "--tmpfs",
        "/var/run/postgresql:rw,nosuid,nodev,uid=999,gid=999,mode=0770,size=8m",
        "--tmpfs",
        "/tmp:rw,nosuid,nodev,mode=1777,size=64m",
        "--env",
        "PGDATA=/var/lib/postgresql/data",
        "--env",
        "POSTGRES_USER=perfect_test",
        "--env",
        "POSTGRES_DB=perfect_test",
        "--env",
        `POSTGRES_PASSWORD=${password}`,
        imageIds[POSTGRES_IMAGE]!,
      ]);
      if (db.code)
        throw new Blocked(
          "POSTGRES_START",
          "No se pudo iniciar PostgreSQL temporal",
        );
      let ready = false;
      for (let i = 0; i < 60; i++) {
      ...
      if (!ready)
        throw new Blocked("POSTGRES_READY", "PostgreSQL no quedó listo");
      intent(app);
      const result = await run(
        [
          ...confinementArgs(app, request.goalId, this.config, network),
          "--env",
          `DATABASE_URL=postgresql://perfect_test:${password}@perfect-postgres.test:5432/perfect_test`,
          "--env",
          "PERFECT_EPHEMERAL_DATABASE=1",
          "--volume",
          `${resolve(dir)}/input:/input:ro`,
          "--volume",
          `${resolve(dir)}/driver.mjs:/driver/execute.mjs:ro`,
          "--volume",
        ...
        Math.min(command.timeoutMs, this.config.limits.timeoutPerTask),
      );
      return {
        code: result.code,
        stdout: result.stdout.replaceAll(password, "[test-secret]"),
        stderr: result.stderr.replaceAll(password, "[test-secret]"),
        artifacts: [],
        environment: {
          database: "PostgreSQL",
          databaseImage: imageIds[POSTGRES_IMAGE],
          appImage: imageIds[appImage],
          ephemeral: true,
          network: "internal",
      ...
      for (const record of [...intents].reverse()) {
        const networkResource = record.resource.startsWith("network:"),
          resource = networkResource
            ? record.resource.slice(8)
            : record.resource;
        const check = await processRun(
          "docker",
          [
            networkResource ? "network" : "container",
            "inspect",
            "--format",
            networkResource
        ...
        } else if (!(await runtime.available()))
          throw new Blocked(
            "RECOVERY_SANDBOX",
            "Docker no responde; se conserva el intent para reconciliarlo",
          );
        this.store.put(
          "intents",
          { ...record, status: "completed" },
          "postgres.resource_stopped",
        );
      }
    }
  }
