...
export function defaultStudio(config: PerfectConfig): StudioConfig {
  const setFor: Record<Role, string> = {
    planner: "general",
    general: "general",
    integrator: "general",
  ...
  const modes: WorkMode[] = [
    ...
    },
    {
      ...
      defaults: { ...defaults, frontend: "motion-remotion" },
      engine: "project",
      instruction:
  ...
  return StudioConfigSchema.parse({
    ...
    skills: {
      ...
      maxCatalogTokens: 1200,
      disabledPackages: ["superpowers"],
      assignments: [],
    },
    sets: [
    ...
    preferences: {
      database: "postgresql",
      backendAsService: false,
      simplicity: "minimal-sufficient",
    },
  });
}
export function resolveProfile(
  ...
  const mode = config.modes.find(
    (m) => m.id === config.activeMode && m.enabled,
  );
  ...
  if (!profile || profile.role !== role || !mode.profiles.includes(wanted))
    throw new Blocked(
      "PROFILE_SCOPE",
      `Perfil no autorizado para ${role}: ${wanted}`,
    );
  const sets = profile.setIds.map((id) => config.sets.find((s) => s.id === id));
  if (sets.some((s) => !s || !s.enabled))
    throw new Blocked("SET_DISABLED", "El equipo está desactivado");
  return {
    profile,
    mode,
    sets: sets.filter((s): s is NonNullable<typeof s> => Boolean(s)),
    role,
  };
}
