...
async function query(requestId: string, raw: unknown) {
  try {
    ...
    if (q.kind === "reference-import") {
      ...
      const path = await safePath(root, q.path), files = (await manifest(root, config)).files.map(f => f.path);
      if (!files.includes(q.path)) throw Error("Archivo fuera de los recursos autorizados");
      const bytes = await readFile(path);
      if (bytes.length > 500000 || bytes.includes(0)) throw Error("El visor de texto admite archivos UTF-8 hasta 500 KB");
      ...
      const bytes = await readEvidence(goal, evidence, 150_000_000), extension = extname(evidence.artifactRef).toLowerCase();
      ...
      const sha256 = createHash("sha256").update(bytes).digest("hex"), dir = join(engine.home, "desktop-media");
...
process.on("message", (raw: unknown) => {
  ...
  if (!input.success) { send({ type: "fault", message: "Mensaje IPC de motor inválido" }); return; }
  const m = input.data;
