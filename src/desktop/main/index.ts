...
function detachPreview() {
  ...
  if (window && !window.isDestroyed())
    window.webContents.send("perfect:event", { type: "preview", open: false });
}
async function launch() {
  ...
  const dev = app.isPackaged
    ? undefined
    : (JSON.parse(
        await readFile(join(app.getAppPath(), "engine-dev.json"), "utf8"),
      ) as { node: string });
  ...
  protocol.handle("perfect", async (request) => {
    try {
      ...
      if (url.host === "app") {
        ...
        if (
          !/^\/(?:index\.html|renderer(?:-[A-Za-z0-9]+)?\.(?:js|css)|chunks\/[A-Za-z0-9_.-]+\.js|assets\/[a-z0-9_.-]+)$/.test(
            path,
          )
        )
  ...
  window.webContents.on("context-menu", (_event, params) => {
    ...
    if (params.isEditable)
      items.push({ role: "cut" }, { role: "paste" }, { role: "selectAll" });
  ...
  const relay = (event: Parameters<EngineBroker["emit"]>[1]) => {
    ...
    if (
      ...
      try {
        const u = new URL(event.message.url);
  ...
  ipcMain.handle("perfect:drop", async (event, raw) => {
    assertSender(event);
    const paths = z.array(z.string().min(1).max(4096)).max(16).parse(raw);
    ...
    for (const p of paths) {
      ...
      if (
        ...
        !st.isFile() ||
        st.size > 25_000_000 ||
        /(?:^|[\\/])(?:\.env|\.ssh|credentials|auth\.json)/i.test(p)
      )
        throw Error("Adjunto privado o incompatible");
      checked.push(await realpath(p));
    }
    const answer = await dialog.showMessageBox(window, {
      type: "question",
      buttons: ["Cancelar", "Seleccionar archivos"],
      defaultId: 0,
      cancelId: 0,
      message: "Seleccionar estos archivos locales",
      detail:
        checked.join("\n") +
  ...
  ipcMain.handle("perfect:request", async (event, raw) => {
    ...
    try {
      if (
        ...
        envelope.workspaceId !== broker.workspaceId
      )
        throw Error("Sesión o carpeta obsoleta: actualizá antes de actuar");
      if (consumed.has(envelope.requestId))
        throw Error("Solicitud duplicada; no se repitió");
      if (consumed.size >= 10000)
        throw Error("Reabrí después de pausar para renovar la sesión");
      consumed.add(envelope.requestId);
      const p = envelope.payload;
      let data: unknown;
      if (
        envelope.operation === "choose-workspace" ||
        envelope.operation === "choose-resource"
      ) {
        const input = z
          .object({ directory: z.boolean().default(false) })
          .parse(p ?? {});
        const directory =
          envelope.operation === "choose-workspace" || input.directory;
        const result = await dialog.showOpenDialog(window, {
          properties: [directory ? "openDirectory" : "openFile"],
          title: directory
            ? "Elegir carpeta"
            : "Seleccionar recurso para revisar",
        });
        const path = result.canceled ? undefined : result.filePaths[0];
        if (path) {
          const canonical = await realpath(path);
          grantedPaths.add(canonical);
          if (envelope.operation === "choose-workspace")
        ...
        for (const path of collectSensitivePaths(a))
          if (!grantedPaths.has(path))
            throw Error(
              "Elegí el recurso con el diálogo nativo antes de utilizarlo",
            );
        const fingerprint = createHash("sha256")
          .update(JSON.stringify(a))
          .digest("hex");
        if (pendingActions.has(fingerprint))
          throw Error("La misma acción ya está en curso");
        pendingActions.add(fingerprint);
        try {
          data = await broker.request("action", a, envelope.requestId);
        } finally {
        ...
        if (!authUrls.has(u))
          throw Error("URL no emitida por el flujo de autenticación");
        const answer = await dialog.showMessageBox(window, {
          type: "question",
          buttons: ["Cancelar", "Abrir navegador"],
          defaultId: 0,
          cancelId: 0,
          message: "Continuar autenticación",
          detail: new URL(u).origin,
        });
        if (answer.response === 1) await shell.openExternal(u);
      } else if (envelope.operation === "copy") {
        clipboard.writeText(
          z
            .object({ text: z.string().max(100000) })
            .strict()
            .parse(p).text,
        );
        ...
        const box = z
          .object({
            ...
            width: z.number().int().min(0).max(4000),
            height: z.number().int().min(0).max(4000),
            visible: z.boolean(),
          })
          .strict()
          .parse(p);
        if (preview) {
          ...
          preview.setBounds({
            x: Math.min(box.x, w),
            y: Math.min(box.y, h),
            width: Math.min(box.width, Math.max(0, w - box.x)),
            height: Math.min(box.height, Math.max(0, h - box.y)),
          });
  ...
  powerMonitor.on("suspend", () => {
    ...
    if (goal && broker.snapshot?.busy)
      void broker
        .request("action", { type: "pause", goalId: goal.id })
        .catch(() => {});
    detachPreview();
    void broker.request("query", { kind: "preview-stop" }).catch(() => {});
  });
    ...
    void (async () => {
      if (broker.snapshot?.busy) {
        const result = await dialog.showMessageBox(window, {
          type: "question",
          buttons: ["Seguir trabajando", "Pausar y salir"],
          defaultId: 0,
          cancelId: 0,
          message: "Hay trabajo activo",
          detail:
            "Perfect conservará checkpoints y esperará la parada del motor.",
        });
      ...
      try {
        ...
        dialog.showErrorBox("Cierre pendiente", String(e));
      }
    })();
  });
