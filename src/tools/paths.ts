...
export function relativePath(path: string): string {
  if (
    ...
    throw new Blocked("PATH_DENIED", path);
  return path;
}
...
export const omitted = (path: string) =>
  sensitive(path) ||
  path
    .split("/")
    .some((p) =>
...
export async function safePath(
  ...
  if (
    relative(base, destination).startsWith(`..${sep}`) ||
    destination === base
  )
    throw new Blocked("PATH_DENIED", path);
  let current = base;
  for (const part of path.split("/")) {
    ...
    try {
      ...
      if (st.isSymbolicLink() || (st.isFile() && st.nlink > 1))
        throw new Blocked("LINK_DENIED", path);
    } catch (e) {
...
export function secretContent(content: string): boolean {
  return /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----|\b(?:sk-[A-Za-z0-9_-]{24,}|gh[pousr]_[A-Za-z0-9]{30,}|AKIA[A-Z0-9]{16})\b/.test(
    content,
  );
}
