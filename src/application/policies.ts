...
export function classifyFailure(message: string): Failure["category"] {
  if (
    /AUTH|401|403|429|quota|QUOTA|MODEL_|REASONING_|ROUTE_|provider|rate.?limit/i.test(
      message,
    )
  )
    return "provider";
  if (/SANDBOX|docker|ENOENT|dependency|not installed/i.test(message))
    return "environment";
  if (/DENIED|PROTECTED|OWNERSHIP|PRIVACY|APPROVAL|BUDGET/i.test(message))
    return "policy";
