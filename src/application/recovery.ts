...
export async function recoverGoal(
  ...
  for (const intent of store
    ...
    if (revision === intent.beforeRevision) {
      store.put(
        "intents",
  ...
  for (const run of store
    ...
    store.put(
      "runs",
      {
        ...
        status: "interrupted",
        endedAt: now(),
        stopReason: "Controller restarted",
      },
      "agent.interrupted",
    );
    budget.interrupt(run.id);
  }
  for (const attempt of store
    .list("attempts", goal.id)
    .filter((a) => a.status === "running"))
    store.put(
      "attempts",
      { ...attempt, status: "interrupted", endedAt: now() },
      "attempt.interrupted",
    );
  ...
  store.put(
    "goals",
    { ...latest, candidateRevision: revision, updatedAt: now() },
    "recovery.completed",
  );
}
