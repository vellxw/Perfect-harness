...
export const TransportSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    headers: z.record(z.string().regex(/^[A-Za-z][A-Za-z0-9-]{0,63}$/), z.string().max(2048)).default({}),
...
export const WindowGrantSchema = z.object({
...
export const IntegrationSchema = z.discriminatedUnion("kind", [
  z.object({ ...common, kind: z.literal("mcp"), transport: TransportSchema,
    tools: z.record(toolName, EffectSchema),
    resourcePrefixes: z.array(z.string().max(2048)).max(32).default([]),
  }).strict(),
  z.object({ ...common, kind: z.literal("github"),
    repositories: z.array(z.string().regex(/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/)).min(1).max(20),
    tokenEnv: envName.default("PERFECT_GITHUB_TOKEN"),
    writeMode: z.enum(["deny", "confirm"]).default("deny"),
    branchPrefix: z.string().regex(/^[a-zA-Z0-9_-]+\/$/).default("perfect/"),
  }).strict(),
  z.object({ ...common, kind: z.literal("browser"),
    maxActions: z.number().int().min(1).max(500).default(100),
  }).strict(),
  z.object({ ...common, kind: z.literal("desktop"), grant: WindowGrantSchema.optional() }).strict(),
]);
...
export interface IntegrationOperation {
  ...
  digest: string;
  logicalKey: string;
  state: "pending" | "approved" | "running" | "completed" | "denied" | "expired" | "unknown" | "reconciled";
  createdAt: string;
  expiresAt: string;
