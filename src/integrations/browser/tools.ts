...
export const BrowserSchemas = {
  browser_open: z
    .object({
      ...
      width: z.number().int().min(240).max(1920).default(1280),
      height: z.number().int().min(240).max(1080).default(800),
    })
    .strict(),
  ...
  browser_fill: z
    .object({ ...observed, value: z.string().max(10000) })
    .strict(),
  browser_select: z
    .object({
      ...observed,
  ...
  browser_press: z
    ...
    .strict(),
  browser_scroll: z
    .object({ deltaY: z.number().int().min(-3000).max(3000) })
    .strict(),
  browser_resize: z
    .object({
...
const descriptions: Record<BrowserTool, string> = {
  browser_open:
    "Iniciar la aplicación de esta tarea en Docker y abrir un navegador aislado. Solo acepta un comando permitido en una copia saneada, no una URL externa ni el perfil personal.",
  browser_snapshot:
    "Observar la página actual: estructura accesible, texto y referencias a elementos. Una observación no equivale a una prueba aprobada.",
  browser_navigate:
    "Navegar a una ruta de la aplicación aislada, conservando la sesión.",
  browser_click:
    "Pulsar un elemento observado. Requiere la referencia y el identificador de la última observación.",
  browser_fill:
    "Completar un campo observado. No permite campos de contraseña ni carga de archivos.",
  ...
  browser_screenshot:
    "Capturar los píxeles reales de la ventana actual; campos privados enmascarados.",
...
export const BROWSER_TOOLS: CatalogTool[] = Object.entries(BrowserSchemas).map(
  ([name, schema]) => ({
    name,
    description: descriptions[name as BrowserTool],
...
export function browserEffect(tool: string): Effect | undefined {
  ...
  return ["browser_snapshot", "browser_screenshot", "browser_console"].includes(
    tool,
  )
    ? "read"
    : "interactive";
