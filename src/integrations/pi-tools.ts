...
export function integrationTools(
  ...
  const wrap = (
    ...
    execute: async (_call, args) => {
      ...
      try {
        ...
        return structured
          ? {
              ...(value as Awaited<ReturnType<RunIntegrations["call"]>>),
  ...
  return [
    ...
    wrap(
      "mcp_call",
      "Ejecutar una herramienta de una integración autorizada. Consultá mcp_list primero. argumentsJson contiene los argumentos del esquema exacto. Las acciones externas pueden esperar aprobación del usuario.",
      Type.Object({
        server: Type.String(),
        tool: Type.String(),
      ...
      async (args) => {
        const input = z
          .object({
            server: z.string(),
            tool: z.string(),
            argumentsJson: z.string().max(2000000),
          })
          .strict()
          .parse(args);
        const parsed = z
          .record(z.string(), z.unknown())
          .parse(JSON.parse(input.argumentsJson));
    ...
    wrap(
      "mcp_resources",
      "Listar recursos de un servidor con prefijos URI autorizados, sin seguir enlaces externos automáticamente.",
      Type.Object({
        server: Type.String(),
      ...
      async (args) => {
        const input = z
          .object({ server: z.string(), cursor: z.string().optional() })
          .strict()
          .parse(args);
        return integrations.resources(input.server, input.cursor);
      },
    ),
    wrap(
      "mcp_read_resource",
      "Leer un recurso MCP explícito del ámbito autorizado. Nunca concede permisos ni se acepta como prueba automática de éxito.",
      Type.Object({ server: Type.String(), uri: Type.String() }),
      async (args) => {
        const input = serverArgs
          .extend({ uri: z.string().max(2048) })
          .parse(args);
        return integrations.readResource(input.server, input.uri);
      },
      true,
    ),
  ];
