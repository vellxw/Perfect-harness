...
export function validationRows(s:UiSnapshot):Row[]{
 ...
 const result:Row[]=[{id:"validation-check",status:"info",title:"Revisar configuración local",detail:"Sin inferencias, compras ni uso del escritorio",body:"Detectado/configurado no equivale a autenticado, invocado o verificado. El diagnóstico no consume cuota."},{id:"validation-cancel",status:"info",title:"Detener pruebas locales",detail:"Cancelar las pruebas activas sin borrar evidencia",body:"Solicita cancelación y conserva resultados parciales."}];
 if(!report)return result;
 result.push({id:"validation-export",status:"info",title:"Exportar informe saneado",detail:`${report.build.version} · ${report.build.platform} · ${report.build.sourceCommit.slice(0,8)}`,body:"Solo estado y procedencia. No incluye claves, prompts, rutas personales ni logs privados."});
