...
export async function startTui(options: TuiOptions = {}): Promise<void> {
  const client: UiClient = options.demo
    ? new DemoClient(options.demo)
    : new EngineClient({ home: options.home, workspace: options.workspace });
  ...
  await new Promise<void>((resolveClose, reject) => {
    ...
    if (options.motion) {
      const prefs = structuredClone(client.getSnapshot().preferences);
      prefs.ui.motion = options.motion;
      if (options.demo)
        client.dispatch({ type: "preferences", preferences: prefs });
      else {
