...
function colorPng(rgb: [number, number, number]): string {
  ...
  ihdr[8] = 8;
  ihdr[9] = 2;
  const row = Buffer.from([0, ...Array.from({ length: 32 }, () => rgb).flat()]);
...
const SmokeSchema = z
  .object({
    ok: z.literal(true),
    firstColor: z.enum(["red", "green", "blue", "white"]),
    secondColor: z.enum(["red", "green", "blue", "white"]),
    summary: z.string(),
  })
  .strict();
...
export async function providerSmoke(input: {
  home: string;
  ...
  const dir = join(input.home, "diagnostics");
  await mkdir(dir, { recursive: true, mode: 0o700 });
  const source = await mkdtemp(join(dir, "provider-source-"));
  // Keep controller state outside the source directory, even for diagnostics.
  const goal = await createGoal(
    {
      request: "Synthetic provider connectivity diagnostic, not a coding goal.",
      source,
      home: input.home,
      config: input.config,
      privacy: "public",
  ...
  const executor = new AgentExecutor(
      ...
      runner,
      input.config,
      input.contributorConsent ?? false,
    ),
    checks: SmokeCheck[] = [];
  for (const role of input.roles ?? [
    ...
    try {
      ...
      const output = SmokeSchema.parse(result.output.result);
      if (output.firstColor !== "blue" || output.secondColor !== "red")
        throw new Error("Image input probe returned incorrect colors");
      const toolEvents = input.store.events(goal.id).filter((event) => {
        const p = event.payload as {
          runId?: string;
          details?: { toolName?: string; isError?: boolean };
        };
        return (
          event.type === "agent.tool_completed" &&
          p.runId === result.run.id &&
          p.details?.toolName === "list_files" &&
          !p.details.isError
        );
      });
      if (!toolEvents.length)
        throw new Error("No successful tool execution was observed");
      const usage = input.store
        .list("usage", goal.id)
        .filter((u) => u.runId === result.run.id);
      ...
      checks.push({
        ...
        reasoningReported: last?.reasoningReported,
        requests: usage.length,
        reportedTokens: usage.reduce((n, u) => n + (u.totalTokens ?? 0), 0),
      });
    } catch (error) {
      checks.push({ role, status: "BLOCKED", detail: errorText(error) });
    }
    if (input.signal.aborted) break;
  }
  const current = input.store.get("goals", goal.id)!;
  input.store.put(
    "goals",
    transition(
      current,
  ...
  const report = join(goal.root, "provider-smoke.json");
  await writeFile(report, JSON.stringify({ at: now(), checks }, null, 2));
  return { goalId: goal.id, report, checks };
}
export async function fullstackSmoke(input: {
  home: string;
  config: PerfectConfig;
  store: StateStore;
  mock: boolean;
  ...
  const dir = join(input.home, "diagnostics");
  await mkdir(dir, { recursive: true, mode: 0o700 });
  const source = await mkdtemp(join(dir, "fullstack-source-"));
  ...
  const finished = await new Orchestrator(input.store, runtime, runner).run(
    goal.id,
    {
      acceptPlan: input.acceptPlan,
      contributorConsent: input.contributorConsent,
      signal: input.signal,
    },
  );
  const observed = [
  ...
  const missing = [
    ...
    "backend",
    "oracle",
  ].filter((role) => !observed.includes(role as Role));
  return {
    status: finished.state === "DONE" && !missing.length ? "PASS" : "BLOCKED",
    mode: finished.mode,
    goal: finished,
