  ...
  const server=createServer(async(req,res)=>{
    try{
      ...
      if(req.method==='POST'&&path==='/api/reservations'){
        ...
        let body='';for await(const chunk of req){body+=chunk.toString();if(body.length>8192)return send(res,413,{error:'Contenido demasiado grande'});}
        let value;try{value=JSON.parse(body);}catch{return send(res,400,{error:'JSON inválido'});}
        if(!value||typeof value.name!=='string'||value.name.trim().length<2||value.name.trim().length>80||!SLOTS.includes(value.slot))return send(res,400,{error:'Nombre u horario inválido'});
        ...
        catch(error){if(error.errcode===2067||String(error.message).includes('UNIQUE constraint'))return send(res,409,{error:'El horario ya está reservado'});throw error;}
      }
    ...
    }catch(error){console.error(error.message);if(!res.headersSent)send(res,500,{error:'Error interno'});else res.end();}
  });
  let closed=false;
...
app.server.listen(Number(process.env.PORT??3000),'0.0.0.0',()=>console.log('Servidor de reservas listo'));
