...
function App(){
 ...
 const board=<section className="game" aria-label="Tablero 3 por 3"><div className="board">{Array.from({length:9},(_,n)=>{const x=n%3,y=Math.floor(n/3),player=game.x===x&&game.y===y;return <div key={n} className={`cell ${n===4?'hazard':''} ${n===8?'target':''}`} aria-label={player?'Jugador':n===4?'Peligro':n===8?'Baliza':'Vacío'}>{player?'●':n===4?'×':n===8?'◇':''}</div>;})}</div></section>;
 return <main>
  ...
  <div style={{maxWidth:330,margin:'26px auto'}}>{reduced?board:<BorderBeam active={game.status==='playing'} size="md" colorVariant="ocean" theme="dark">{board}</BorderBeam>}</div>
