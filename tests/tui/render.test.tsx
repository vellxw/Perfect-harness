...
for (const [width, height] of [
  ...
  test(`native TUI remains usable at ${width}x${height}`, async () => {
    ...
    try {
      ...
      assert.match(frame, /Iteration/);
      assert.match(frame, /Verification/);
      assert.match(frame, /Ask Perfect|Working/);
      assert.match(frame, /DEMO/);
      assert.match(frame, /Repairing the responsive layout/);
      assert.doesNotMatch(frame, /Add agent|CPU|MEM|NET/);
      const lines = frame.split("\n");
      assert.ok(lines.length <= height + 1);
      if (width >= 130) assert.match(frame, /AGENTS/);
...
for (const screen of [
  ...
  test(`native ${screen} overlay renders and returns home`, async () => {
    ...
    try {
      ...
      await app.flush();
      assert.match(app.captureCharFrame(), /CURRENT GOAL/);
    } finally {
      await app.dispose();
    }
  });
...
for (const scene of [
  ...
  test(`native fixture ${scene} is clearly identified`, async () => {
    ...
    try {
      const frame = app.captureCharFrame();
      assert.match(frame, /DEMO/);
      if (scene === "idle") assert.match(frame, /What do you want to build/);
      if (scene === "done") assert.match(frame, /done/i);
      if (scene === "paused") assert.match(frame, /Docker is unavailable/);
    } finally {
      await app.dispose();
    }
  });
test("palette is keyboard-driven and uses the real selected command", async () => {
  ...
  try {
    ...
    await app.flush();
    assert.match(app.captureCharFrame(), /Commands/);
    app.mockInput.pressKey("RETURN");
    await app.flush();
    assert.match(app.captureCharFrame(), /Agents/);
...
test("abort is not dispatched by an accidental Enter", async () => {
  ...
  try {
    ...
    app.mockInput.pressKey("RETURN");
    await app.flush();
    assert.match(app.captureCharFrame(), /Type ABORT/);
    app.mockInput.pressKey("RETURN");
    await app.flush();
...
test("resize preserves composer and main navigation", async () => {
  ...
  try {
    for (const [w, h] of [
      ...
      app.resize(w!, h!);
      await app.flush();
      assert.match(app.captureCharFrame(), /Ask Perfect/);
