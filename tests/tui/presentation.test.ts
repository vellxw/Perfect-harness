...
test("SQLite wakeups occur after commit and never after rollback", async () => {
  ...
  let wakes = 0;
  const off = onDatabaseChange(path, () => wakes++);
  try {
    assert.throws(() =>
      store.transaction(() => {
        store.event("g", "discard", {});
    ...
    store.transaction(() => {
      store.event("g", "first", {});
      store.transaction(() => store.event("g", "second", {}));
    });
    await Promise.resolve();
    assert.equal(wakes, 1);
    assert.equal(store.events("g").length, 2);
    assert.equal(store.tailEvents("g", 1)[0]!.type, "second");
  } finally {
    off();
    store.close();
...
test("SQLite nested failure rolls back to savepoint, not the outer transaction", () => {
  const store = new SqliteStore(":memory:");
  try {
    store.transaction(() => {
      store.event("g", "retained", {});
      assert.throws(() =>
        store.transaction(() => {
          store.event("g", "discarded", {});
          throw Error("nested");
        }),
      );
      store.event("g", "also-retained", {});
    });
    assert.deepEqual(
      store.events("g").map((e) => e.type),
      ["retained", "also-retained"],
    );
  } finally {
    store.close();
  }
});
test("UI cannot directly set DONE or bypass destructive confirmations", () => {
  assert.equal(
    UiActionSchema.safeParse({ type: "done", goalId: "g" }).success,
    false,
  );
  assert.equal(
    UiActionSchema.safeParse({ type: "abort", goalId: "g" }).success,
    false,
  );
  assert.equal(
...
test("terminal control sequences are data, never executable output", () => {
  const result = text("\x1b]52;c;c2VjcmV0\x07Hello\x1b[2J world");
...
test("task ordering retains a join and its complete dependencies", () => {
  const tasks = demoSnapshot().tasks,
    result = orderedTasks(tasks),
    join = result.find((r) => r.task.id === "integrate")!;
  assert.equal(join.depth, 3);
  assert.equal(result.filter((r) => r.task.id === "integrate").length, 1);
  assert.equal(join.task.dependencies.length, 2);
});
test("large lists are windowed and palette filtering is bounded", () => {
  const input = Array.from({ length: 20000 }, (_, i) => i);
  const view = viewportRows(input, 19999, 12);
  assert.equal(view.rows.length, 12);
  assert.equal(view.rows.at(-1), 19999);
  assert.equal(filterCommands("/agent")[0]!.name, "agents");
});
test("motion auto honors reduced-motion and remote settings", () => {
  assert.equal(motionEnabled("off", {}), false);
  assert.equal(motionEnabled("auto", { SSH_CONNECTION: "x" }), false);
  assert.equal(motionEnabled("auto", { PERFECT_REDUCED_MOTION: "1" }), false);
  assert.equal(motionEnabled("full", {}), true);
});
test("engine is event-driven and refuses unknown/cross-workspace actions", async () => {
  const root = await mkdtemp(join(tmpdir(), "perfect-engine-")),
    workspace = join(root, "source");
  await mkdir(workspace);
  const messages: UiMessage[] = [];
  const engine = await PresentationEngine.create(
    { home: join(root, "home"), workspace },
    (m) => messages.push(m),
  );
  try {
    ...
    await engine.dispatch("scope", {
      type: "select",
      goalId: "other-workspace",
    });
    assert.ok(
      messages.some(
        (m) => m.type === "result" && m.requestId === "scope" && !m.ok,
      ),
    );
    const count = messages.filter((m) => m.type === "snapshot").length;
    await new Promise((r) => setTimeout(r, 150));
    assert.equal(messages.filter((m) => m.type === "snapshot").length, count);
  } finally {
