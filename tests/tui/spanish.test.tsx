...
for (const screen of ["agents", "plan", "tasks", "verify", "artifacts", "routing", "cost", "logs", "doctor", "settings", "projects"] as Screen[]) {
  test(`pantalla ${screen}: textos propios en español y sin etiquetas inglesas heredadas`, async () => {
    ...
    try {
      ...
      assert.doesNotMatch(frame, /CURRENT GOAL|YOUR NEXT IDEA|Quick actions|Unknown command|not observed|unknown \(not exposed\)|Connect your provider|High contrast|Terminal background|Select a goal|Subscription OAuth|reported tokens|uncertain requests/);
