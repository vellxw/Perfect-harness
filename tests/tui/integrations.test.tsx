...
import {
  ...
  integrationRows,
} from "../../src/ui/tui/integration-view.js";
...
test("conectar GitHub propone un permiso explícito y no lo concede automáticamente", () => {
  const snapshot = demoSnapshot("idle"),
    result = integrationCommand("github", "vellxw/Perfect-harness", snapshot);
  assert.equal(result?.phrase, "CONFIAR");
  assert.equal(result?.action?.type, "integration");
  const action = result!.action!;
  if (action.type !== "integration" || action.action.command !== "configure")
  ...
  assert.deepEqual(action.action.config.repositories, [
    "vellxw/Perfect-harness",
  ]);
    ...
    pending: [
      {
        ...
        arguments:
          '{"owner":"vellxw","repo":"Perfect-harness","head":"perfect/fix"}',
      },
    ],
  };
  const row = integrationRows(s)[0]!;
  assert.match(row.body, /perfect\/fix/);
  const intent = integrationRowIntent(row.id, s);
  assert.equal(intent?.phrase, "AUTORIZAR");
  assert.match(intent!.body!, /ARGUMENTOS COMPLETOS/);
