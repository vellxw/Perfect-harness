...
import { GuidedForm } from "../../src/ui/tui/form.js";
  ...
  for (const screen of [
    ...
    test(`pantalla V4 ${screen} ${width}×${height} renderiza sin perder cierre`, async () => {
      const store = new SqliteStore(":memory:"),
        client = new DemoClient("running");
      client.getSnapshot().studio = new StudioAdmin(
        store,
        "/synthetic/state",
      ).panel("/synthetic/project", defaultConfig());
      ...
      try {
        root.render(
          <App client={client} initialScreen={screen} onExit={() => {}} />,
        );
        await new Promise((r) => setTimeout(r, 70));
        await ui.renderOnce();
        const frame = ui.captureCharFrame();
        assert.match(frame, /Esc/);
        assert.equal(frame.includes("Ask Perfect"), false);
        assert.ok(frame.trim().length > 200);
      } finally {
        root.unmount();
        ui.renderer.destroy();
        store.close();
...
test("formulario guiado 80×24 permite llegar al último campo y confirmar con teclado", async () => {
  ...
  const fields = Array.from({ length: 12 }, (_, i) => ({
    key: "field-" + i,
    label: "Campo " + (i + 1),
    initial: i === 11 ? "Español ñ á" : "valor",
  }));
  try {
    root.render(
      <GuidedForm
    ...
    assert.match(ui.captureCharFrame(), /Revisar y continuar/);
    ui.mockInput.pressKey("RETURN");
    await flush();
    assert.equal(saved?.["field-11"], "Español ñ á");
  } finally {
    root.unmount();
    ui.renderer.destroy();
  }
