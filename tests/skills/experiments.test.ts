...
test("comparador separa calidad, regresión, empate y ambos-fallan", () => {
  assert.equal(pairVerdict(true, true), "both-pass");
  assert.equal(pairVerdict(false, true), "improvement");
  assert.equal(pairVerdict(true, false), "regression");
  assert.equal(pairVerdict(false, false), "both-fail");
});
test("borrador se autoriza solo para evaluación sin aprobación de producción", () => {
  const store = new SqliteStore(":memory:"),
    base = defaultConfig(),
    registry = new SkillsRegistry(store),
    workspace = "/fixture/eval";
  try {
    registry.get(workspace, base);
    const r = releaseFromFiles({
      files: {
    ...
    registry.quarantine(r, workspace);
    const spec = {
      releaseId: r.id,
      profileId: "backend",
      partition: "holdout",
      cases: [
        {
          id: "dev",
          kind: "response",
          prompt: "Explicar consulta segura",
        ...
        },
        {
          id: "holdout",
          kind: "response",
          prompt: "Cómo ejecutar SQL seguro",
          partition: "holdout",
          contains: ["parámetro"],
        },
      ],
    };
    const t = proposeTrial(store, workspace, base, spec);
    assert.equal(t.status, "proposed");
    assert.throws(
      () => authorizeTrial(store, t.id, t.specHash, workspace, [], "EVALUAR"),
      /SKILL_INSPECT_FIRST/,
    );
    const seen = inspectReceipt(store, workspace, r);
    const allowed = authorizeTrial(
      store,
      t.id,
      t.specHash,
      workspace,
    ...
    assert.equal(
      store.list("skillSelections").some((s) => s.releaseId === r.id),
      false,
    );
    const report = trialReport(store, t.id);
    assert.equal(report.complete, false);
    assert.equal(report.qualityPassed, false);
    registry.setMaster(false);
    assert.throws(
      () => proposeTrial(store, workspace, base, spec),
      /SKILLS_OFF/,
    );
  } finally {
    store.close();
...
test("contrato de evaluación reserva casos y limita cuotas; no acepta JSON arbitrario como resultado real", () => {
  ...
  try {
    assert.throws(
      () => trialReport(store, "imported-real-report"),
      /EVAL_UNKNOWN/,
    );
  } finally {
    store.close();
