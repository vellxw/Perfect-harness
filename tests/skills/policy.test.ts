...
test("equipos filtran conocimientos aunque usan el mismo modelo", () => {
  const config = defaultStudio(defaultConfig()),
    backend = resolveProfile(config, "backend").profile,
    frontend = resolveProfile(config, "frontend").profile;
  frontend.binding = structuredClone(backend.binding);
  const skill = builtinSkills().find(
    (s) => s.skillId === "perfect-postgres-backend",
  )!;
  const selection = {
    id: "selection",
    goalId: "registry",
  ...
  config.skills.assignments.push({
    skillId: skill.skillId,
    scope: "global",
    decision: "enable",
  });
...
test("Superpowers no reaparece por asignación global y el reviewer nunca escribe", () => {
  const c = defaultStudio(defaultConfig()),
    p = resolveProfile(c, "general").profile,
    r = builtinSkills()[0]!;
  const s = {
    id: "s",
    goalId: "r",
  ...
  c.skills.assignments.push({
    skillId: r.skillId,
    scope: "global",
    decision: "enable",
  });
  assert.equal(
    skillAccess(c, p, { ...r, packageId: "superpowers" }, s).allowed,
    false,
  );
...
test("perfiles independientes conservan responsabilidad, equipos y binding", () => {
  const c = defaultStudio(defaultConfig());
  c.activeMode = "game-creator";
  const game = resolveProfile(c, "general");
  assert.equal(game.profile.id, "gameplay");
  assert.deepEqual(game.profile.setIds, ["gameplay"]);
  assert.equal(profileDefinition(game).id, "general");
  assert.throws(
    () => resolveProfile(c, "backend", "gameplay"),
    /PROFILE_SCOPE/,
  );
  assert.throws(
    () => resolveProfile(c, "general", "motion-ae"),
    /PROFILE_SCOPE/,
  );
...
test("SKILL.md admite YAML real, rechaza alias/duplicados y verifica cada byte", () => {
  ...
  assert.ok(releaseBytes(r, "SKILL.md").length);
  const changed = structuredClone(r);
  changed.files["SKILL.md"]!.base64 = Buffer.from("cambio").toString("base64");
  ...
  for (const path of [
    ...
    assert.throws(() => safeResource(path));
...
test("activadores distinguen tarea y exclusiones y no fuerzan procedimientos", () => {
  ...
  assert.equal(
    matchesTask({ ...skill, excludes: ["sin beam"] }, "diseño sin beam"),
    false,
  );
...
test("entorno MCP preserva nombres Windows sin heredar credenciales ni flags", () => {
  ...
  assert.throws(
    () => cleanEnvironment("C:\\safe", { Path: "OTHER" }, { OTHER: "x" }),
    /MCP_ENV_DENIED/,
  );
});
