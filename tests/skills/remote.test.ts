...
function transport(truncated = false, tamper = false) {
  const files: { [key: string]: Buffer } = {
    "skills/demo-skill/SKILL.md": content,
    "skills/demo-skill/references/check.md": Buffer.from(
  ...
  return async (url: string) => {
    if (url.includes("/git/trees/"))
      return Buffer.from(
        JSON.stringify({
          truncated,
          tree: Object.entries(files).map(([path, data]) => ({
            path,
            type: "blob",
            mode: "100644",
            size: data.length,
    ...
    const name = decodeURIComponent(
      url.split("/" + input.commit + "/")[1] ?? "",
    );
    const data = files[name];
    if (!data) throw Error("Unexpected fetch " + url);
    return tamper && name.endsWith("SKILL.md")
...
test("importador usa solo commit fijado, conserva licencia y rechaza árbol incompleto", async () => {
  const r = await importGithubSkill(
    input,
    new AbortController().signal,
    transport(),
  );
  assert.equal(r.provenance.commit, input.commit);
  assert.equal(r.provenance.redistribution, "allowed");
  assert.deepEqual(r.defaultSets, ["backend"]);
  assert.ok(releaseBytes(r, "SKILL.md").equals(content));
  ...
  await assert.rejects(
    () =>
      importGithubSkill(input, new AbortController().signal, transport(true)),
    /TRUNCATED/,
  );
});
test("contenido no coincide con Git blob, fuentes excluidas y licencias desconocidas se bloquean", async () => {
  ...
  await assert.rejects(
    () =>
      importGithubSkill(
        { ...input, repository: "chuspeeism/dashi-motion" },
        new AbortController().signal,
        transport(),
      ),
    /LICENSE/,
  ...
  await assert.rejects(
    () =>
      importGithubSkill(
        { ...input, repository: "supabase/agent-skills" },
        new AbortController().signal,
        transport(),
      ),
    /EXCLUDED/,
