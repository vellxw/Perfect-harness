...
test("real Pi SDK: explicit route, tool call, events and cancellation", async () => {
  ...
  const server = createServer(async (req, res) => {
    ...
    res.write(
      `data: ${JSON.stringify({ id: "chatcmpl-contract", object: "chat.completion.chunk", model: "contract-model", choices: [{ index: 0, delta, finish_reason: null }] })}\n\n`,
    );
    res.write(
      `data: ${JSON.stringify({ id: "chatcmpl-contract", object: "chat.completion.chunk", model: "contract-model", choices: [{ index: 0, delta: {}, finish_reason: called ? "stop" : "tool_calls" }], usage: { prompt_tokens: 20, completion_tokens: 5, total_tokens: 25 } })}\n\n`,
    );
    res.end("data: [DONE]\n\n");
  });
  ...
  const probe: ToolDefinition = {
    name: "probe",
    label: "Probe",
    description: "Call this tool once.",
  ...
  const session = await openSession({
    ...
    reasoning: "off",
    systemPrompt: "Call probe then answer.",
    tools: [probe],
  });
  const events: string[] = [];
