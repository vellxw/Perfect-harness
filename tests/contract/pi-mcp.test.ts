...
test(
  "Pi AgentSession real descubre MCP, ejecuta una herramienta stdio y conserva su observación",
  { timeout: 30000 },
  async () => {
    const root = await mkdtemp(join(tmpdir(), "perfect-pi-mcp-")),
    ...
    const discovered = await probeIntegration(home, workspace, "test", signal);
    registry.authorizeCatalog(workspace, "test", discovered.catalog!.hash);
    const sequence = [
      { name: "mcp_list", arguments: { server: "test" } },
      {
        ...
        arguments: {
          server: "test",
          tool: "echo",
          argumentsJson: JSON.stringify({ text: "Pi y MCP integrados: ñ" }),
        },
      },
      {
        name: "submit_result",
        arguments: {
          json: JSON.stringify({ summary: "Contrato cumplido", outputs: [] }),
        },
      },
    ];
    const server = createServer(async (req, res) => {
      ...
      const item = instruction
        ...
        : {
            ...
            role: "assistant",
            content: [
              { type: "output_text", text: "Finalizado", annotations: [] },
            ],
            status: "completed",
          };
    ...
    try {
      ...
      request.run.routeBinding = await runtime.resolve(
        {
          id: "general",
      ...
      assert.ok(
        registry
          ...
          .some((o) => o.tool === "echo" && o.state === "completed"),
      );
    } finally {
      registry.close();
