...
import { scopedWindowsArguments, literalDesktopCommand } from "../../src/integrations/desktop/arguments.js";
...
test("texto de escritorio no puede sustituir HWND ni convertirse en flags", () => {
  for (const tool of ["desktop_type", "desktop_set_value"] as const) {
    ...
    const args = scopedWindowsArguments("123", literalDesktopCommand(tool, "MessageInput", payload));
    assert.deepEqual(args.slice(0, 5), ["ui", tool === "desktop_type" ? "send-keys" : "set-value", "-w", "123", "--json"]);
    assert.deepEqual(args.slice(-2), ["--", payload]);
    assert.equal(args.filter(v => v === "-w").length, 1);
  }
  assert.throws(() => literalDesktopCommand("desktop_type", "--window=999", "x"));
});
test("errores remotos desconocidos no vuelcan secretos y los fallos de permisos conservan su código", () => {
  ...
  assert.match(integrationFailure({ status: 401 }).message, /MCP_AUTH_REQUIRED/);
  assert.match(integrationFailure({ statusCode: 429 }).message, /MCP_RATE_LIMIT/);
...
test("cancelar herramienta nativa espera su cierre antes de devolver el control", async () => {
  const home = await mkdtemp(join(tmpdir(), "perfect-native-stop-")), pidFile = join(home, "pid.txt"), abort = new AbortController();
  let pid = 0;
  try {
    const pending = nativeProcess(process.execPath, ["-e", "require('node:fs').writeFileSync(process.argv[1],String(process.pid));setInterval(()=>{},1000)", pidFile], home, abort.signal);
    for (let n = 0; n < 100 && !pid; n++) { await delay(20); try { pid = Number(await readFile(pidFile, "utf8")); } catch {} }
    assert.ok(pid);
