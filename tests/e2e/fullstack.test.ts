...
test('Docker fullstack: real SQLite, real browser, mobile failure, repair, independent roles and DONE',{skip:!enabled,timeout:300000},async()=>{
  ...
  try{
    ...
    if(finished.state!=='DONE')console.error(JSON.stringify({goal:finished.state,reason:finished.pauseReason??finished.terminalReason,failures:store.list('failures',goal.id),checks:store.list('verifications',goal.id),tasks:store.list('tasks',goal.id).map(t=>({id:t.id,status:t.status,error:t.failureReason}))},null,2));
    ...
    if(goalId&&process.env.PERFECT_ARTIFACT_DIR){
      const out=resolve(process.env.PERFECT_ARTIFACT_DIR);await mkdir(out,{recursive:true});const goal=store.get('goals',goalId)!;
      for(const e of store.list('evidence',goalId).filter(e=>['screenshot','report','trace'].includes(e.kind))){
        const name=e.revision===goal.candidateRevision?basename(e.artifactRef):`${e.id}-${basename(e.artifactRef)}`;
        await copyFile(e.artifactRef,join(out,name)).catch(()=>{});
      }
      await writeFile(join(out,'fullstack-summary.json'),JSON.stringify({state:goal.state,mode:goal.mode,reason:goal.pauseReason??goal.terminalReason,roles:runtime.roles,maximumParallelWriters:runtime.maximumParallelWriters,iterations:goal.iteration,verifications:store.list('verifications',goalId),failures:store.list('failures',goalId),events:store.events(goalId).map(e=>({type:e.type,at:e.occurredAt}))},null,2));
    }
