...
test(
  "PostgreSQL real aislado: Fastify, SQL, concurrencia, reinicio API y rollback; nunca SQLite",
  { skip: process.env.PERFECT_TEST_DOCKER !== "1", timeout: 420000 },
  async () => {
    const root = await mkdtemp(join(tmpdir(), "perfect-pg-")),
    ...
    const runner = new PreparedDockerRunner(
      config,
      store,
      join(home, "sandbox"),
    );
    ...
    try {
      const goal = await createGoal(
        {
          request: "Probar backend PostgreSQL propio",
          source,
          home,
          config,
          privacy: "public",
      ...
      assert.match(result.summary, /PostgreSQL/);
      assert.equal(
      ...
      if (process.env.PERFECT_ARTIFACT_DIR) {
        const dir = resolve(process.env.PERFECT_ARTIFACT_DIR, "postgres");
